...
dir <- "/Users/khawajahassan/BA21_Coding/Assignment_3/"
...
data$ind2_cat <- as.factor(data$ind2_cat)

data_services <- data %>% filter(ind2 >= 55 & ind2 <= 56)

data_manufacturing <- data %>% filter(ind2 >= 26 & ind2 <= 33)
...
rf_predicted_probabilities_services <- predict(rf_model_p, newdata = data_services, type = "prob")
data_services$rf_p_prediction <- rf_predicted_probabilities_services[,"fast_growth"]
RMSE(data_services$rf_p_prediction, data_services$fast_growth)

# ROC curve on services
roc_obj_services <- roc(data_services$fast_growth, data_services[, "rf_p_prediction", drop=TRUE])

# AUC
as.numeric(roc_obj_services$auc)

# Get expected loss on holdout with optimal threshold
services_treshold <- coords(roc_obj_services, x = best_tresholds[["rf_p"]] , input= "threshold",
                            ret="all", transpose = FALSE)
expected_loss_services <- (services_treshold$fp*FP + services_treshold$fn*FN)/length(data_services$fast_growth)
expected_loss_services

# Confusion table on holdout set 
services_prediction <-
  ifelse(data_services$rf_p_prediction < best_tresholds[["rf_p"]] , "no_fast_growth", "fast_growth") %>%
  factor(levels = c("no_fast_growth", "fast_growth"))
cm_object_rf_service<- confusionMatrix(services_prediction,data_services$fast_growth_f)
cm_rf_service <- cm_object_rf_service$table
cm_rf_service
...
rf_predicted_probabilities_manufacturing <- predict(rf_model_p, newdata = data_manufacturing, type = "prob")
data_manufacturing$rf_p_prediction <- rf_predicted_probabilities_manufacturing[,"fast_growth"]
RMSE(data_manufacturing$rf_p_prediction, data_manufacturing$fast_growth)

# ROC curve on services
roc_obj_manufacturing <- roc(data_manufacturing$fast_growth, data_manufacturing[, "rf_p_prediction", drop=TRUE])

# AUC
as.numeric(roc_obj_manufacturing$auc)

# Get expected loss on holdout with optimal threshold
manu_treshold <- coords(roc_obj_manufacturing, x = best_tresholds[["rf_p"]] , input= "threshold",
                        ret="all", transpose = FALSE)
expected_loss_manu <- (manu_treshold$fp*FP + manu_treshold$fn*FN)/length(data_manufacturing$fast_growth)
expected_loss_manu

# Confusion table on holdout set 
manu_prediction <-
  ifelse(data_manufacturing$rf_p_prediction < best_tresholds[["rf_p"]] , "no_fast_growth", "fast_growth") %>%
  factor(levels = c("no_fast_growth", "fast_growth"))
cm_object_rf_manu<- confusionMatrix(manu_prediction,data_manufacturing$fast_growth_f)
cm_rf_manu <- cm_object_rf_manu$table
cm_rf_manu
cm_rf_service
...
create_calibration_plot(data_manufacturing, 
                        file_name = "RF-calibration-service", 
                        prob_var = "rf_p_prediction", 
                        actual_var = "fast_growth",
                        n_bins = 20)
data_manufacturing$rf_p_prediction
create_calibration_plot(data_services, 
                        file_name = "RF-calibration-service", 
                        prob_var = "rf_p_prediction", 
                        actual_var = "fast_growth",
                        n_bins = 20)

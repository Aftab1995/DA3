...
t1 <- as.data.frame.matrix(cm1_perc)
...
# Introduce loss function(based on the defined loss function Fp= -3.3% Interest Rate , Fn= -6.6% Interest Rate)
...
t2 <- as.data.frame.matrix(cm_rf_perc)

tm <- cbind(t1,t2)


combined_cm <-kable(tm,"latex",longtable =T,booktabs =T,caption ="Confusion Table Combined Dataset",digits = 2)%>%
